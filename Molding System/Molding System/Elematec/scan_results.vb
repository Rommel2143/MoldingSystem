...
Public Class scan_results
    ...
    Public Sub loaddata(text As String)
        ...
        Select Case status
            Case "IN"
                ...
                grp_total.Text = "Total QTY IN"
            Case "OUT"
                btn_report.Visible = True
                grp_total.Text = "Total QTY OUT"
        End Select
    End Sub
        ...
        Try
            Select Case status
                Case "IN"
                    con.Close()
                    con.Open()
                    Dim cmdselect As New MySqlCommand("Select distinct  CONCAT(`firstname`,' ',`last`) as fullname FROM `molding_stock`
            ...
            Select Case status
                ...
                Case "OUT"
                    ...
                    Dim cmdselect As New MySqlCommand("SELECT DISTINCT ts.`batchout` 
                                               ...
                                               AND CONCAT(`firstname`, ' ', `last`) = @fullname and status=0", con)
    ...
    Private Sub refreshgrid_in()
        Try
            con.Close()
            con.Open()
            Dim cmdrefreshgrid As New MySqlCommand("SELECT `partname`,ts.partcode,  `lotnumber`, `remarks`, `qty` FROM `molding_stock` ts
                                                    JOIN molding_masterlist fm ON ts.partcode=fm.partcode
            ...
            con.Close()
            con.Open()
            Dim cmdrefreshgrid2 As New MySqlCommand("SELECT `partname`, SUM(`qty`) AS TOTAL FROM `molding_stock` ts
                                                    JOIN molding_masterlist fm ON ts.partcode=fm.partcode
                                                    LEFT JOIN trc_user tsoout  ON ts.userin = tsoout.IDno
    ...
    Private Sub refreshgrid_out()
        Try
            ...
            Dim cmdrefreshgrid As New MySqlCommand("SELECT `partname`,ts.partcode,  `lotnumber`, `remarks`, `qty` FROM `molding_stock` ts
                                                    ...
                                                    WHERE `dateout`='" & dtpicker.Value.ToString("yyyy-MM-dd") & "' and CONCAT(`firstname`, ' ', `last`)='" & cmbuser.Text & "' and `batchout`='" & cmb_batch.Text & "'  and status=0", con)
            ...
            con.Close()
            con.Open()
            Dim cmdrefreshgrid2 As New MySqlCommand("SELECT `partname`,ts.partcode, SUM(`qty`) FROM `molding_stock` ts
                                                    JOIN molding_masterlist fm ON ts.partcode=fm.partcode
                                                    LEFT JOIN trc_user tsoout  ON ts.userout = tsoout.IDno
                                                    WHERE `dateout`='" & dtpicker.Value.ToString("yyyy-MM-dd") & "' and CONCAT(`firstname`, ' ', `last`)='" & cmbuser.Text & "' and `batchout`='" & cmb_batch.Text & "' and status=0
                                                    GROUP BY ts.partcode", con)
